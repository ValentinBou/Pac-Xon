...
public class KenneyToLibGdxAtlasConverter implements Runnable {
    ...
    public void run() {
        try {
            System.out.println("starting at [" + rootPath + "]...");
    ...
    private void convertKennyXML(Path file) throws IOException {
        ...
        Files.write(newPath, sb.toString().getBytes(), StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING);
    ...
    public static void main(String[] args) {
        if (args.length != 1) { printUsage(); exit(1); }
        try {
            KenneyToLibGdxAtlasConverter converter = new KenneyToLibGdxAtlasConverter(true, Paths.get(args[0]));
            converter.run();
